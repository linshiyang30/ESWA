...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        for epoch in range(args.epoch):
            ...
            pred_val = (logits[val_idx].cpu().numpy()>0).astype(int)
            pred_test = (logits[test_idx].cpu().numpy()>0).astype(int)
            test_acc  = (pred_test == labels[test_idx].cpu().numpy()).mean()
            micro = f1_score(labels[val_idx].cpu().numpy(), pred_val, average='micro')
            macro = f1_score(labels[val_idx].cpu().numpy(), pred_val, average='macro')
            t_end = time.time()
            # print validation info
            print('Epoch {:05d} | Val_Loss {:.4f} | Val_acc {:.4f} | Test_acc {:.4f} | Macro_F1 {:.4f} |Time(s) {:.4f}'.format(
                epoch, val_loss.item(),micro, test_acc, macro, t_end - t_start))
            # early stopping
